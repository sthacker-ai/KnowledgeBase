...
```mermaid
flowchart LR
    A[Default Claude Mode] --> B{User Invokes Slash Command};
    B --> C[Claude Adopts Agent Persona];
    C --> D{Command Function};
    D --> E[Execute Specialized Task];
    E --> F[Strategic Output];
    F --> G[Improved Decision Making];
```
