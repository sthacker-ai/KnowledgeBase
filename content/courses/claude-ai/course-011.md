...
```mermaid
flowchart LR
    A[AI Generates Code] --> B{Code contains Waste/Redundancy};
    B --> C[Developer Crafts 65-Line Markdown Prompt];
    C --> D[Prompt Guides LLM towards Quality Output];
    D --> E[AI Generates Optimized Code];
    E --> F[Measure Mistake Rate Reduction];
    F --> G[Result: Reduced Code Waste];
```
