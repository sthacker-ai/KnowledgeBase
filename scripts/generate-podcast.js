...
// Strip any chain-of-thought reasoning preamble that some models (e.g. nemotron) leak.
// Finds the first line starting with [ALEX]: or [SAM]: and returns from there.
function extractDialogue(raw) {
  const lines = raw.split("\n");
  const firstTag = lines.findIndex((l) => /^\[(ALEX|SAM)\]:/.test(l.trim()));
  if (firstTag === -1) return null; // no dialogue found
  return lines.slice(firstTag).join("\n").trim();
}
...
- ALEX opens with a specific claim, result, contradiction, or practical problem from the course
- Never open with "Imagine", "Picture this", "What if", "Today we", "In this episode", or generic radio-host language
- Vary the opening style. Make it sound like a smart person starting a useful conversation, not a trailer.
- SAM asks clarifying questions, spots assumptions, and asks for concrete examples
...
- Prefer concrete examples, tradeoffs, and how-to details over hype
- Output ONLY the dialogue. Start immediately with [ALEX]:`;

  const userMsg = `Course: ${title}\nTopic: ${topicLabel}\n\n---\n${content.slice(0, 3500)}`;
  const raw = await orChat(system, userMsg, 1600);
  const dialogue = extractDialogue(raw);
  if (!dialogue) throw new Error("AI response contained no [ALEX]/[SAM] dialogue — model leaked reasoning chain");
  return dialogue;
}
