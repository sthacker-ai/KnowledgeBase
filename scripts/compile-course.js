...
function buildCourseMessages(rawJson, extractedJson, transcript) {
  ...
  const title        = extractedJson.title || extractedJson.page_title || tweetText.slice(0, 120);
  ...
  const codeBlocks = Array.isArray(extractedJson.code_blocks)
    ? extractedJson.code_blocks.filter((block) => typeof block === "string" && block.trim())
    : [];
  const codeBlocksContent = codeBlocks.length
    ? codeBlocks.slice(0, 20).map((block, index) => `CODE BLOCK ${index + 1}:\n${block.slice(0, 2000)}`).join("\n\n")
    : "";
...
${codeBlocksContent ? `EXTRACTED CODE BLOCKS:\n${codeBlocksContent}\n` : ""}
---
...
async function main() {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (raw.course_path) {
      // Overwrite the existing course file for transcript backfills and forced re-runs.
    ...
    fs.mkdirSync(path.dirname(courseFullPath), { recursive: true });
