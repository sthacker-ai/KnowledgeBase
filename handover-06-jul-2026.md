...
## Addendum — the site went live

Vercel deployed at `kb.thinkbits.in`, R2 media and Neon Postgres both
connected and verified working end-to-end. Two real bugs surfaced and got
fixed along the way:

- **Migration bug**: `pg`'s default parameter serialization turns a plain
  JS array into a Postgres native-array literal, not JSON text — broke
  inserting `pipeline_runs.steps` (a JSON array) into Neon. Fixed with an
  explicit `JSON.stringify()` before insert in `migrate-to-neon.js`.
  Re-ran cleanly; all 1,978 rows verified present with correct row counts
  and JSON integrity independently re-checked against Neon directly.
- **Vercel env var mismatch**: the user set a "kb" prefix when provisioning
  Neon through Vercel's integration, so Vercel injected `KB_DATABASE_URL`
  instead of plain `DATABASE_URL` — the app only ever reads the latter.
  Fixed by adding a second, correctly-named `DATABASE_URL` var with the
  same value.
- **Media manifest never pushed**: `data/media-manifest.json` (436 hero
  images + 423 podcasts, generated by the R2 upload script) was sitting
  uncommitted locally while the user redeployed — Vercel built with the old
  empty manifest, so no media showed up. Committed and pushed; confirmed
  live afterward (both a hero image and a podcast URL return `200` with
  correct content-type directly from R2).
- **`/admin` was publicly browsable** on the hosted deployment (noticed by
  the user after deploying, not caught during earlier testing). Fixed with
  `middleware.ts`: 404s the entire `/admin` + `/api/admin/*` surface when
  `process.env.VERCEL` is set, verified working in both directions
  (`VERCEL=1` locally → 404; without it → 200).
- **R2 upload + git publish are now pipeline steps 11–12**, not manual
  commands — `scheduled-daily.js` uploads new media and pushes to `main`
  itself at the end of every daily run, so the hosted site updates with no
  human step. Deliberate tradeoff, stated plainly in the docs: this is an
  unattended push straight to the branch Vercel deploys from.
