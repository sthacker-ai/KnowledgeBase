...
```mermaid
flowchart LR
    A[Identify Codebase Challenge] --> B{Internal Strategy & Tooling Setup};
    B --> C[Prompt Engineering & Context Injection];
    C --> D[Codex Query/Analysis];
    D --> E{Code Understanding & Pattern Recognition};
    E --> F[Generate Refactoring Strategy];
    F --> G[Apply Refactoring (Test & Review)];
    G --> H[Achieve Codebase Refactoring Goal];
```
