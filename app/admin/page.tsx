...
// ─────────────────────────────────────────────────────────────────────────────
// Media generation steps (still available as individual triggers)
// ─────────────────────────────────────────────────────────────────────────────

const MEDIA_STEPS = [
  { id: "transcripts",      label: "Transcribe Videos",  color: "#f97316", desc: "Download & transcribe video sources" },
  { id: "generate-assets",  label: "Generate Images",    color: "#a855f7", desc: "AI hero images for all courses (Replicate Flux Schnell)" },
  { id: "generate-podcasts",label: "Generate Podcasts",  color: "#ec4899", desc: "2-speaker podcast audio per course (Kokoro 82M)" },
];
...
export default function AdminPage() {
  ...
  // ── Run quick pipeline (with tweet limit) ────────────────────────────────
  const runQuickPipeline = async (limit: number) => {
    if (isRunning) return;
    setActiveStep("pipeline");
    setIsRunning(true);
    setTaskOutput(`[Starting pipeline: process up to ${limit} tweets]\n`);
    try {
      const res = await fetch("/api/admin/run", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ step: "pipeline", extraArgs: ["--limit", String(limit)] }),
      });
      const data = await res.json();
        ...
        <div className="kb-source-panel">
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <p style={{ fontSize: "0.75rem", color: "var(--muted)", margin: "0 0 10px" }}>
              Runs the full pipeline (import → extract → classify → courses → wiki → graph) for N new tweets.
            </p>
            <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <label style={{ fontSize: "0.8rem", color: "var(--muted)" }}>Tweets:</label>
                <input
                  type="number"
                  min={1}
                  max={25}
                  value={tweetLimit}
                  onChange={(e) => setTweetLimit(Math.min(25, Math.max(1, Number(e.target.value))))}
                  disabled={isRunning}
                  style={{
                ...
                />
                <span style={{ fontSize: "0.72rem", color: "var(--muted)" }}>max 25</span>
              </div>
              <button
                onClick={() => runQuickPipeline(tweetLimit)}
                disabled={isRunning}
                style={{
              ...
              >
                {isRunning && activeStep === "pipeline" ? "Pipeline Running…" : `Run Pipeline (${tweetLimit} tweets)`}
              </button>
            </div>
          </div>
          ...
          <button
            ...
            style={{
              width: "100%",
              background: isRunning ? "var(--border)" : "#1e293b",
              color: "#fff", border: "1px solid var(--border)", borderRadius: "8px",
              padding: "9px", fontWeight: 600, fontSize: "0.82rem",
              cursor: isRunning ? "not-allowed" : "pointer",
              marginBottom: "10px",
            }}
          >
            {isRunning && activeStep === "pipeline" ? "Pipeline Running…" : "Run Full Pipeline (no limit — all pending)"}
          </button>

          {/* Media generation steps */}
          <p style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--muted)", margin: "14px 0 8px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
            Media Generation
          </p>
