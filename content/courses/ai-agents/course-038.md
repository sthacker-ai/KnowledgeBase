...
```mermaid
flowchart LR
    A[Start: Define Persistent Goal (/goal)] --> B(AI Agent Initialization);
    B --> C{Decompose Goal into Sub-tasks};
    C --> D(Execute First Sub-task);
    D --> E(Monitor Progress & State);
    E --> F{Goal Achieved?};
    F -- No --> G(Re-plan / Adjust Strategy);
    G --> D;
    F -- Yes --> H(Final Output / Completion);
```
