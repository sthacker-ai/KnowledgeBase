...
async function main() {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const isYouTube = ["youtube", "youtube_video"].includes(ext.extraction_type);
    const tweetUrl = isYouTube
      ? (ext.page_url || raw.source_url || raw.tweet_url || "")
      : (raw.tweet_url || ext.tweet_url || ext.page_url || "");
