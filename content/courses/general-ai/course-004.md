...
```mermaid
flowchart LR
    A[Define Hard Academic Problem] --> B[Design Context & Prompt];
    B --> C[GPT-5.4 Pro Execution];
    C --> D[Generate Solution];
    D --> E[Review & Refine Output];
    E --> F[Convert to LaTeX Format];
    F --> G[Publish/Final Review];
```
