...
```mermaid
flowchart LR
    A[User Defines Goal] --> B{Craft Prompt};
    B --> C[Send Request via API];
    C --> D[Claude Processes Request];
    D --> E[Receive AI Output];
    E --> F{Trigger External Action};
    F --> G[Integrate with Tools (e.g., Zapier/Make)];
