...
```mermaid
flowchart LR
    A[Start: Initial Prompt/Task] --> B{Execute Chain Step};
    B --> C[Claude Processes Information];
    C --> D[Self-Reflection/Assessment];
    D --> E{Decision: Refine or Continue?};
    E -- Continue --> F[Generate Next Prompt/Action];
    F --> B;
    E -- Refine --> G[Iterate Prompt Chain];
    G --> B;
```
