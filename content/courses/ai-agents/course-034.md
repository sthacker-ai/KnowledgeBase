...
```mermaid
flowchart LR
    A[Define Persistent Goal] --> B[Gather Evidence/Context]
    B --> C{Evaluate Progress Toward Goal}
    C -->|Not Met| D[Select Next Useful Action]
    D --> E[Execute Action]
    E --> B
    C -->|Goal Met| F[Terminate / Output Result]
```
