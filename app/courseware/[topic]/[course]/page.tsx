...
import { getCourseMediaUrls } from "../../../lib/media";
...
export default async function CoursePage({ params }: Props) {
  ...
  const hasTranscript = String(frontmatter.has_transcript ?? "false") === "true";
  ...
  // Check for generated hero image and podcast audio — resolves to a local
  // /public path in dev, or an R2 URL from the media manifest when hosted
  // (see app/lib/media.ts).
  const { heroUrl, podcastUrl } = getCourseMediaUrls(topic, course);
  const hasHeroImage = heroUrl !== null;
  const hasPodcast   = podcastUrl !== null;
    ...
    <main className="kb-shell">
      ...
      <section className="kb-workspace">
        ...
        {/* Source card */}
          ...
          {/* Podcast audio player — shown right after source card / tweet embed */}
          {hasPodcast && (
            <div style={{
              padding: "14px 18px",
              marginBottom: "20px",
              ...
              <p style={{ fontWeight: 700, fontSize: "0.85rem", margin: "0 0 8px" }}>
                🎙 Podcast Version
              </p>
              <audio controls style={{ width: "100%" }}>
                {podcastUrl && <source src={podcastUrl} type="audio/mpeg" />}
                Your browser does not support audio playback.
              </audio>
              {hasScript && (
                <p style={{ fontSize: "0.72rem", color: "var(--muted)", marginTop: "6px", marginBottom: 0 }}>
        ...
        {/* Prev / Next navigation + Completion */}
        <div style={{ display: "flex", gap: "12px", margin: "16px 0", alignItems: "center", flexWrap: "wrap" }}>
          {prevCourse ? (
          ...
          <div style={{ flex: 1 }} />
          <CompletionButton courseId={`${topic}/${course}`} topicSlug={topic} />
          {nextCourse ? (
