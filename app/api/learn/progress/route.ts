/**
 * GET  /api/learn/progress?courseId=xxx — check completion status
 * POST /api/learn/progress              — upsert learning progress
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    const rows = await query<{ status: string }>(
      "SELECT status FROM learning_progress WHERE course_id = $1 LIMIT 1",
      [courseId]
    );
