...
```mermaid
flowchart LR
    A[Backtest Strategy Returns] --> B{Define Null Hypothesis: No Edge};
    B --> C[Repeatedly Permute Data];
    C --> D[Calculate Performance Metric for Each Permutation];
    D --> E[Generate Null Distribution];
    E --> F[Calculate P-value];
    F --> G{P-value vs. Significance Level?};
    G -- Reject H0 --> H[Validate Real Edge];
    G -- Fail to Reject H0 --> I[Data Mining/Overfitting];
```
