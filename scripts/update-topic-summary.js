...
async function main() {
  ...
  for (let i = 0; i < topicFolders.length; i++) {
    ...
    // Skip if summary already exists with the same course count — nothing new to summarize
    const summaryPath = path.join(COURSES_DIR, topicSlug, "summary.md");
    if (fs.existsSync(summaryPath)) {
      try {
        const existingRaw = fs.readFileSync(summaryPath, "utf8");
        const existing = matter(existingRaw);
        if (Number(existing.data.course_count) === courses.length) {
          console.log(`[summary]   topic=${topicSlug} up-to-date (${courses.length} courses) — skipping`);
          runLog.results.push({ topic: topicSlug, status: "skipped" });
          continue;
        }
    ...
    const topicDir     = path.join(COURSES_DIR, topicSlug);
    const writePath    = path.join(topicDir, "summary.md");
    fs.writeFileSync(writePath, frontmatter + summaryMarkdown.trim() + "\n", "utf8");
