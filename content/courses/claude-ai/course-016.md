...
```mermaid
flowchart LR
    A[Start with Vague Goal] --> B{Define Role / Context};
    B --> C[Apply Chain-of-Thought Reasoning];
    C --> D[Consider Token Budgeting];
    D --> E[Draft Initial Prompt];
    E --> F{Review & Refine};
    F -- Needs Improvement --> C;
    F -- Ready --> G[Final Polished Prompt];
```
