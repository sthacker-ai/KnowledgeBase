...
const NVIDIA_MODELS    = (process.env.NVIDIA_MODELS || "mistralai/mistral-nemotron,meta/llama-3.1-8b-instruct")
...
function openRouterChat(model, messages, maxTokens) {
  return new Promise((resolve, reject) => {
    ...
    let _orTimedOut = false;
    const _orTimer = setTimeout(() => { _orTimedOut = true; req.destroy(new Error(`OpenRouter timeout [${model}] after 1 min`)); }, 60000);
    ...
    req.on("error", (e) => {
      clearTimeout(_orTimer);
      if (_orTimedOut) {
        reject(new Error(`OpenRouter timeout [${model}] after 1 min`));
      } else {
        reject(new Error(`OpenRouter network error [${model}]: ${e.message}`));
      }
    });
    req.write(body);
    req.end();
...
// ---------------------------------------------------------------------------
// Internal: NVIDIA NIM chat completions — streaming mode (SSE)
//
// Why streaming? NVIDIA free-tier has a ~38s proxy timeout that drops
// non-streaming connections mid-wait. With streaming, the server sends the
// first token within a few seconds, keeping the connection alive throughout.
// This function accumulates all SSE delta chunks and resolves with the full
// assembled content — callers see no difference from the old non-streaming version.
// ---------------------------------------------------------------------------
...
function nvidiaChat(model, messages, maxTokens) {
  return new Promise((resolve, reject) => {
    ...
    let usage         = null;
    let settled       = false;
    let rawBuffer     = "";  // partial SSE line buffer

    // Rolling stale-stream timer: resets on every chunk received.
    // If no data arrives for 45s at any point, give up.
    let _staleTimer = null;
    const STALE_MS  = 45000;
    const resetStale = () => {
      clearTimeout(_staleTimer);
      _staleTimer = setTimeout(() => {
        if (!settled) { settled = true; req.destroy(); reject(new Error(`NVIDIA NIM stale stream [${model}] — no data for 45s`)); }
      }, STALE_MS);
    };

    const req = https.request(options, (res) => {
      // Non-200 responses won't be SSE — read as plain text and reject.
        ...
        res.on("end", () => {
          ...
          if (!settled) {
            ...
            try {
              ...
              reject(new Error(`NVIDIA NIM error [${model}]: ${parsed.error?.message || errBody.slice(0, 200)}`));
            } catch {
              reject(new Error(`NVIDIA NIM HTTP ${res.statusCode} [${model}]: ${errBody.slice(0, 200)}`));
            }
          }
        });
        return;
      ...
      resetStale();  // start the stale timer once connected

      res.on("data", (chunk) => {
        resetStale();  // keep resetting as long as data flows
        rawBuffer += chunk.toString();

        // SSE lines are separated by \n. Process all complete lines.
        const lines = rawBuffer.split("\n");
        rawBuffer = lines.pop();  // keep any incomplete last line for next chunk
        ...
        for (const line of lines) {
          ...
          try {
            ...
            // Some providers send usage in the final stream chunk
            if (parsed.usage) usage = parsed.usage;

            // Check for error inside SSE
            if (parsed.error) {
              if (!settled) {
                settled = true;
                clearTimeout(_staleTimer);
      ...
      res.on("end", () => {
        ...
        if (!settled) {
          settled = true;
          if (!content) {
            reject(new Error(`NVIDIA NIM returned no content [${model}]`));
          } else {
            resolve({ content: content.trim(), usage });
          }
        }
      });
    });
    ...
    req.on("error", (e) => {
      ...
      if (!settled) {
        settled = true;
        reject(new Error(`NVIDIA NIM network error [${model}]: ${e.message}`));
      }
    });

    resetStale();  // also start timer before connection is established
    req.write(body);
    req.end();
  });
}
...
async function aiGenerate(messages, opts = {}) {
  ...
  if (OPENROUTER_API_KEY && !_openRouterDead) {
    ...
    try {
      ...
      if (isRateLimitError(err)) {
        console.warn(`  [${label}] OpenRouter rate-limited (${err.message}) — skipping for rest of this run`);
        ...
        try {
          ...
          if (isRateLimitError(fallbackErr)) {
            console.warn(`  [${label}] OpenRouter fallback rate-limited (${fallbackErr.message}) — skipping for rest of this run`);
            _openRouterDead = true;
          } else {
  ...
  if (NVIDIA_API_KEY && !_nvidiaDead) {
    for (const nimModel of NVIDIA_MODELS) {
      try {
        console.log(`  [${label}] Trying NVIDIA NIM: ${nimModel}`);
        const { content: result, usage } = await nvidiaChat(nimModel, messages, maxTokens);
        if (result && result.length > 50) {
          console.log(`  [${label}] NVIDIA NIM OK (${result.length} chars)`);
          recordTokenUsage(label, nimModel, "NVIDIA-NIM", usage);
          return result;
        }
        throw new Error("Response too short");
      } catch (err) {
        if (isRateLimitError(err)) {
          console.warn(`  [${label}] NVIDIA NIM rate-limited (${err.message}) — skipping all NIM for rest of this run`);
          _nvidiaDead = true;
          break;
        }
        console.warn(`  [${label}] NVIDIA NIM [${nimModel}] failed: ${err.message}`);
        if (err.message.includes("timeout") || err.message.includes("stale stream")) {
          console.warn(`  [${label}] NVIDIA NIM [${nimModel}] timed out — trying next model`);
        }
        // Any non-rate-limit failure: continue to next model in list
      }
    }
    if (!_nvidiaDead) {
      console.warn(`  [${label}] All NVIDIA NIM models exhausted — going to Ollama`);
      _nvidiaDead = true;
    }
  } else if (_nvidiaDead) {
    console.log(`  [${label}] NVIDIA NIM dead — going straight to Ollama`);
  }
...
function aiProviderInfo() {
  ...
  const nvidia  = NVIDIA_API_KEY     ? `NVIDIA NIM (${NVIDIA_MODELS.join(" → ")})` : null;
