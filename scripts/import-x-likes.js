...
// Patterns are loaded at startup from data/config/relevance-filters.json.
// Edit via Admin > Topic Relevance Filters — no code change needed.
// Only import tweets that relate to: AI, technology, self-help, philosophy,
// religion, leadership, tools/tech, investment, stock market, trading.
// ---------------------------------------------------------------------------

const FILTERS_CONFIG_PATH = path.join(ROOT, "data", "config", "relevance-filters.json");

const RELEVANT_PATTERNS = (() => {
  try {
    if (!fs.existsSync(FILTERS_CONFIG_PATH)) {
      console.warn("[import] relevance-filters.json not found — all tweets will be imported");
      return [];
    }
    const raw = JSON.parse(fs.readFileSync(FILTERS_CONFIG_PATH, "utf8"));
    const patterns = Array.isArray(raw.patterns) ? raw.patterns : [];
    if (patterns.length === 0) {
      console.warn("[import] No relevance filter patterns found — all tweets will be imported");
      return [];
    }
    console.log(`[import] Loaded ${patterns.length} relevance filter patterns`);
    return patterns.map((p) => new RegExp(p, "i"));
  } catch (err) {
    console.warn(`[import] Failed to load relevance filters: ${err.message} — all tweets will be imported`);
    return [];
  }
})();

function isRelevantTweet(tweet) {
  if (RELEVANT_PATTERNS.length === 0) return true;

  const text = (tweet.tweet_text || "").toLowerCase();

  // No text but has media/links — include (could be educational video or article)
  if (text.trim().length < 15) {
    return (tweet.video_urls?.length > 0 || tweet.outbound_urls?.length > 0);
  }

  // Must match at least one relevant topic keyword
...
function buildConfig() {
  ...
  return {
    ...
    maxScrolls: asNumber(process.env.SCRAPE_MAX_SCROLLS, 200),
...
function loadFilteredIndex() {
  const index = readJson(FILTERED_INDEX_PATH, { schema_version: 1, tweet_ids: {} });
  if (!index.tweet_ids || typeof index.tweet_ids !== "object") index.tweet_ids = {};
  return index;
}

function saveFilteredIndex(index) {
  index.updated_at = new Date().toISOString();
  ensureDir(path.dirname(FILTERED_INDEX_PATH));
  writeJson(FILTERED_INDEX_PATH, index);
}
...
async function scrapeLikedTweets(config, seenIndex, filteredIndex) {
  ...
  try {
    ...
    for (let scroll = 0; scroll < config.maxScrolls; scroll += 1) {
      ...
      for (const tweet of visibleTweets) {
        ...
        // Topic relevance filter — mark irrelevant tweets as seen so we skip them next time
        if (!isRelevantTweet(tweet)) {
          const filteredAt = new Date().toISOString();
          seenIndex.tweet_ids[tweet.tweet_id] = {
            first_seen_at: filteredAt,
            tweet_url: tweet.tweet_url,
            status: "filtered",
            note: "Skipped — not relevant to knowledge base topics",
          };
          filteredIndex.tweet_ids[tweet.tweet_id] = {
            filtered_at: filteredAt,
            tweet_url: tweet.tweet_url,
            author_handle: tweet.author_handle,
            tweet_text: (tweet.tweet_text || "").slice(0, 280),
            reason: "no_topic_match",
          };
          console.log(`[import] FILTERED @${tweet.author_handle}: ${(tweet.tweet_text || "").slice(0, 80).replace(/\n/g, " ")}`);
          filteredCount++;
          continue;
        }
    ...
    if (filteredCount > 0) {
      console.log(`[import] filtered=${filteredCount} tweets (not relevant to knowledge base topics)`);
    }
    return collected;
  } finally {
    await context.close();
...
async function runImport(config) {
  ...
  const filteredIndex = loadFilteredIndex();

  console.log(`[import] handle=@${config.targetHandle} limit=${config.limit} mode=${config.mode}`);
  console.log(`[import] seen=${Object.keys(seenIndex.tweet_ids).length} filtered-total=${Object.keys(filteredIndex.tweet_ids).length}`);

  const tweets = await scrapeLikedTweets(config, seenIndex, filteredIndex);
  const saved = [];
  ...
  saveFilteredIndex(filteredIndex);
