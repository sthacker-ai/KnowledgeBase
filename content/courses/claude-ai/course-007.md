...
```mermaid
flowchart LR
    A[Start with Underspecified Prompt] --> B{Model Output?};
    B -- Incorrect/Guesswork --> C[Observe Failure];
    C --> D[Refine Prompt: Add Context/Tone/Delimiters];
    D --> E[Add Few-Shot Examples/Step-by-Step Instructions];
    E --> F[Test New Prompt];
    F -- High Confidence Response --> G[Achieve Desired Behavior];
    F -- Failure --> C;
```
